...
export function validateHttpsUrl(value) {
    ...
    try {
        ...
        return { url: null, error: 'Link must be a valid https URL' };
    }

    if (parsed.protocol !== 'https:') {
        return { url: null, error: 'Link must start with https://' };
    }

    if (parsed.username || parsed.password) {
        return { url: null, error: 'Link cannot include login credentials' };
    }
...
export function formatIdrDisplay(value) {
    if (!value) {
        return '';
    }
    ...
    const digits = parseIdrInput(trimmed);
