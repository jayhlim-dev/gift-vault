import { getCollection } from './firebase-admin';
...
export async function getPersons({ limit } = {}) {
    try {
        const persons = await getCollection('persons', { limit });
...
export async function getNotes({ limit } = {}) {
    try {
        const notes = await getCollection('notes', { limit });
...
export async function getWishlists({ limit } = {}) {
    try {
        return await getCollection('wishlists', { limit });
    } catch (error) {
