...
export function sortCompletedReminders(reminders) {
    return reminders
        ...
        .sort(
            (a, b) =>
                (toDate(b.completedAt)?.getTime() || toDate(b.dueDate)?.getTime() || 0) -
                (toDate(a.completedAt)?.getTime() || toDate(a.dueDate)?.getTime() || 0)
        );
}
...
export function getUpcomingItemDisplay(item, person) {
    const personName = formatDisplayName(person?.name || item.label);
    ...
    return {
        title: item.label,
        subtitle: person ? `${personName} · ${item.dateText}` : item.dateText
    };
}
